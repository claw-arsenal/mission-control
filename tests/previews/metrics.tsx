...
function Preview() {
  ...
  return <div className={dark ? "dark" : ""}><div className="h-screen bg-background text-foreground" style={{ fontFamily: "Arial, sans-serif", "--header-height": "4rem" } as React.CSSProperties}>
    <nav aria-label="Fixture controls" className="flex flex-wrap items-center gap-3 border-b p-2 text-sm"><span>Local sample data</span><button className="rounded border px-3 py-1" onClick={() => { setDark(!dark); document.documentElement.classList.toggle("dark", !dark); }}>Toggle theme</button><button className="rounded border px-3 py-1" onClick={() => { offline = !failed; setFailed(!failed); }}>{failed ? "Restore connection" : "Simulate connection failure"}</button></nav>
    <TooltipProvider><SidebarProvider><div className="flex h-[calc(100vh-48px)] min-w-0 flex-1 flex-col"><MetricsClient /></div></SidebarProvider></TooltipProvider><Toaster />
  </div></div>;
}
