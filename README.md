...
**2026-09-07:** Skill-aware modules now preserve data when disabled. Configure Outlook and Telegram review alerts in Settings ? Notifications. A resident worker checks for published reviews with a default 60-second interval. See [setup and delivery behavior](docs/review-alerts.md).
...
> **Earlier changes (2026-06-08):** The **Mobile Applications** module (toggleable from Settings → Modules; route `/mobile-apps`) tracks your own apps' **App Store + Google Play reviews and ratings** through the **official store APIs only** — the Google Play **Android Publisher API** (`reviews.list`, service account) and the Apple **App Store Connect API** (`customerReviews`, ES256 JWT). It is strictly **view-only**: it reads reviews and ratings and never replies. Credentials are loaded from a `secrets.env` file (service-account JSON path + `.p8` key path), never from the database and never exposed to the browser; see `secrets.env.example`. The per-app detail shows a per-store rating masthead, a star distribution, a review-based rating-trend chart, and a **paginated, filterable, searchable reviews feed** with per-review **Translate to Dutch** (free, key-less MyMemory public API) and **View in store** links. **Headline ratings are the official store values, never self-computed:** Apple's displayed per-storefront average comes from Apple's official **iTunes Lookup** JSON API (`averageUserRating`/`userRatingCount` per country). Google Play's Reviews API only returns written reviews (rating-only feedback is excluded), so official Google ratings come from the **Play Console ratings report** CSVs in your private Cloud Storage bucket (`GOOGLE_PLAY_REPORTS_BUCKET`, read with the same service account via `@google-cloud/storage`) — per-country averages, posted daily with a 3–7 day delay. Without a bucket configured, Google falls back to the fetched-review average (labeled). Google reports give averages but no per-country counts. The same bucket — accessed strictly **read-only** via the `devstorage.read_only` scope (the integration only lists/downloads CSVs; it never uploads, creates buckets, copies report files, or uses BigQuery) — also feeds **installs** (active installs, daily installs/uninstalls), **stability** (daily crashes/ANRs), and **store performance** (store-listing visitors + acquisitions ÷ visitors conversion, per country) shown in a "Play Console reports" card. Report failures (bad bucket, permission denied, no data yet) surface as warnings on the store card instead of being swallowed. `GOOGLE_PLAY_REPORTS_BUCKET` accepts the plain id, a `gs://` URI, or a full path. Traffic-source / search-term store-performance breakdowns are deferred pending a confirmed CSV format. Sync is **automatic on page load** (stale-while-revalidate) with SSE live updates and a short de-dupe window so the APIs aren't hammered. **On-demand agent sentiment digests** ("AI sentiment") reuse the same local `openclaw agent` dispatch as Processes simulation, so the `openclaw` CLI must be on the server PATH. Dependencies: `@googleapis/androidpublisher`, `google-auth-library`, `@google-cloud/storage`, `jose`, `zod`, `p-limit` — run `npm install`. No scraping or unofficial libraries are used.

> **Previous (2026-06-04):** Kanban tickets can now link **more than internal documents**. The link picker gained two tabs alongside the document tree: **URL** (attach any `http(s)` link with an optional label) and **File / Folder** (attach a local path like `M:\Altinstar\2026\AI`). Clicking a path link opens it in **Windows File Explorer on the client** via a custom `mc-explorer:` URL scheme — works even when the server runs on remote Linux, after a one-time per-machine setup ([download `install-mc-explorer.ps1`](public/install-mc-explorer.ps1) from the File/Folder tab and run it; registers under `HKCU`, no admin). URL and path links live in a new `ticket_links` table gated under the Documents module. See [CHANGELOG.md](./CHANGELOG.md) for full list.
